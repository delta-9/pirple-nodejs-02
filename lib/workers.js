...
workers.gatherAllChecks = function() {
  fileStore.list('checks', function(err, checkIds) {
    if (err || !checkIds || !checkIds.length) {
      return debug('Error: no checks to process');
    }
    checkIds.forEach(checkId => {
      fileStore.read('checks', checkId, function(err, checkData) {
        if (err || !checkData) {
          return debug('Error when retrieving a check');
        }
        if (workers.validateCheckData(checkData)) {
          workers.performCheck(checkData);
        } else {
          return debug('Error a check failed data validation');
        }
      });
    });
  });
...
workers.processCheckOutcome = function(originalCheckData, checkOutcome) {
  ...
  fileStore.update('checks', newCheckData.id, newCheckData, function(err) {
    if (err) {
      return debug('Error when updating a check status');
    }
    if (alertWarranted) {
      return workers.alertUserToStatusChange(newCheckData);
    } else {
      return debug('Check outcome has not changed');
    }
  });
};
...
workers.log = function(originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) {
  ...
  logStore.append(logFilename, logString, function(err) {
    if (!err) {
...
workers.rotateLogs = function() {
  logStore.list(false, function(err, logs) {
    if (err || !logs) {
      return debug('Could not find any logs to rotate');
    }
    logs.forEach(function(logName) {
      const newFileId = logName+'-'+Date.now();
      logStore.compress(logName, newFileId, function(err) {
        if (err) {
          return debug('Error could not compress the log file: ' + err);
        }
        logStore.truncate(logName, function(err) {
          if (err) {
            return debug('Error could not truncate the log file');
          }
