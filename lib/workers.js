  ...
  logStore.list(false, function(err, logs) {
    ...
    logs.forEach(function(logName) {
      ...
      logStore.compress(logName, newFileId, function(err) {
        if (err) {
          return console.log('Error could not compress the log file: ' + err);
        }
        logStore.truncate(logName, function(err) {
          if (err) {
            return console.log('Error could not truncate the log file');
          }
          console.log('Success rotating log file');
        });
      });
    });
  });
