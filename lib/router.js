...
router.route = function(path, data, res) {
  if (_handlers[data.method] && _handlers[data.method][path]) {
    return _handlers[data.method][path](data, response(data, res));
  }
  _handlers.default(data, response(data, res));
}

/**
 * Return a response handler for the current route (encapsulate data, res).
 * @param {Object} data
 * @param {Object} res
 */
function response(data, res) {
  return function(status, payload = {}) {
    if ([200, 201].includes(status)) {
      debug('\x1b[32m%s\x1b[0m', data.method.toUpperCase() + '/' + data.path + ' ' + status);
    } else {
      debug('\x1b[31m%s\x1b[0m', data.method.toUpperCase() + '/' + data.path + ' ' + status);
    }
